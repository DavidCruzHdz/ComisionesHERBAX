...
namespace Comisiones.Controllers
{
    ...
    public class ConfiguracionPremiosController : Controller
    {
        private string nameController = "ConfiguracionPremiosController";//nombre que se usa para generar elArchivo LOG (Archivo LOG por controlador)
        ...
        public ActionResult Index()
        {
            ...
            List<CO_st_BuscaProductosReembolsos_Result> LProductos = new List<CO_st_BuscaProductosReembolsos_Result>();

            using (herbaxEntities hbxEntities = new herbaxEntities())
            {
                try
                {
                    LProductos = hbxEntities.CO_st_BuscaProductosReembolsos().ToList();
        ...
        public ActionResult Index(IList<CO_st_BuscaProductosReembolsos1_Result> entity, IList<CO_st_BuscaProductosReembolsos1_Result> entity2)
        {
            ...
            try
            {
               ...
               // var Periodo = PeriodosEvo.Id_Periodo;
                ...
                //var PeriodosEvoPT = EvoPT.CU_tb_Periodos.Where(x => x.Anio == AnioEvo && x.Mes == MesEvo && x.Quincena == QuincenaEvo && x.Id_Empresa == varEmpresa).FirstOrDefault();
                //var PeriodosEvoPT = EvoPT.CU_tb_Periodos.Where(x => x.Anio == AnioEvo && x.Mes == MesEvo && x.Quincena == QuincenaEvo).FirstOrDefault();
                var PeriodoPT = PeriodosEvoPT.Id_Periodo;

                //EvolutionEntities EvoPT = new EvolutionEntities();
                //var PeriodosEvoPt = EvoPT.CO_tb_Periodo.Where(x => x.Id_Periodo == varPeriodo).FirstOrDefault();

                //var GeneraPedido = db.CO_st_GeneraMovimientosPedido(varFolio, varSocio, 70, varEmpresa, varDiferencia, varReembolso, varImpuesto, varTotal, NumPartida, varVentas, varUsuario, AnioEvo, MesEvo, QuincenaEvo);
                // este guarda los datos generales una tabla de pedidos
                // y el otro guarda en un ciclo cada partida
